...
export default function ConfirmUserDelete({ showAlert, setShowAlert, userId, setChange }) {
  const handleUserDelete = (ev) => {
    console.log("asd");
    setShowAlert(false);
    axios.post("/delete_user", { userId }).then(({ data }) => {})
    .then(({data}) =>{
        setChange(true)
    })
  };
      ...
      additionalContent={
        <React.Fragment>
          <div className="mt-2 mb-4 text-sm text-red-700 dark:text-red-800">
            This action will permanently delete this user from the database. Are you sure you want to continue? (Note you will need to refresh this table)
          </div>
          <div className="flex">
            <button
              type="button"
